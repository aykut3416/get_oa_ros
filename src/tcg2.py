...
def get_laser(msg):
    ...
    if coords.shape[0]!=0:
        ...
        if np.min(sqr_dist) > (robot_r*1.5):
            ...
            v_limit = max_v
            v = max((m.pi / 4.0 - abs(alfa)) / (m.pi / 4.0), 0) * v_limit
            if alfa / (m.pi / 2.0) > 1.0:
                w = 1.0
            elif alfa / (m.pi / 2.0) < -1.0:
                w = -1.0
            else:
                w = alfa / (m.pi / 2.0)
            control_signal = [v, w]
            ...
            path_to_goal='low'
            dbscan = DBSCAN(eps=0.3, min_samples=3).fit(coords)
            ...
            v_limit=m.sqrt(1-((safe_v-min_dist)/safe_v))* max_v
            v = max((m.pi / 4.0 - abs(alfa)) / (m.pi / 4.0), 0) * v_limit
            if alfa / (m.pi / 2.0) > 1.0:
                w = 1.0
            elif alfa / (m.pi / 2.0) < -1.0:
