    ...
    if coords.shape[0]!=0:
        ...
        if np.min(sqr_dist) > (robot_r*1.5):
            ...
            if (m.pi / 2.0 + angle1) > 0.2:
                ...
                new_p = [pose[0]+distance * m.cos(pose[2] - m.pi / 2.0), pose[1]+distance * m.sin(pose[2] - m.pi / 2.0)]
            ...
            for ind, [ang1, ang2] in enumerate(angles):
                ...
                if min(a_diffs) < min_angle:
                    ...
                    if min_ind[1] == 1 and a12_diff < 0.0:
                        ...
                        push_theta = ang2 + m.atan((robot_r + safe_d) / close_dist)
                        ...
                        close_cor = gaps[min_ind[0]][min_ind[1]]
                        close_dist = m.hypot(close_cor[0] - pose[0], close_cor[1] - pose[1])
                        push_theta = ang1 - m.atan((robot_r + safe_d) / close_dist)
                        scs_theta = m.atan2(m.sin(push_theta), m.cos(push_theta))
                        min_c_angle = ang1
            ...
            if min_reading > safe_v:
                ...
                for ind in range(N):
                    ...
                    ti=(safe_v-di)/(safe_v)
                    rep_angle=m.atan2(m.sin(near_angles[ind]+m.pi-last_angle),m.cos(near_angles[ind]+m.pi-last_angle))
                    sigma=ti*rep_angle
                    wi=1.0/((1-ti)**3)
                    if anglev<0:
                        W_r+=wi
...
def calculate_alpha(pos,arr,ang_diff):
    ...
    beta=2.0*m.asin(robot_r/min(dist_l,dist_r))
...
if __name__ == '__main__':
    rospy.init_node('cg_test', anonymous=True)
